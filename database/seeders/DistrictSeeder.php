<?php

namespace Database\Seeders;

use App\Models\District;
use Illuminate\Database\Seeder;

class DistrictSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $districts = [
            [
                "district_id"=>"1",
                "region_id"=>"14",
                "name"=>"город Шахрисабз"
            ],
            [
                "district_id"=>"2",
                "region_id"=>"3",
                "name"=>"Папский район"
            ],
            [
                "district_id"=>"3",
                "region_id"=>"2",
                "name"=>"Асакинский район"
            ],
            [
                "district_id"=>"4",
                "region_id"=>"5",
                "name"=>"Ташкентский район"
            ],
            [
                "district_id"=>"5",
                "region_id"=>"5",
                "name"=>"город Янгийуль"
            ],
            [
                "district_id"=>"6",
                "region_id"=>"5",
                "name"=>"город Ахангаран"
            ],
            [
                "district_id"=>"7",
                "region_id"=>"5",
                "name"=>"город Нурафшон"
            ],
            [
                "district_id"=>"8",
                "region_id"=>"11",
                "name"=>"город Нукус"
            ],
            [
                "district_id"=>"9",
                "region_id"=>"11",
                "name"=>"Тахиаташский район"
            ],
            [
                "district_id"=>"10",
                "region_id"=>"4",
                "name"=>"город Хива"
            ],
            [
                "district_id"=>"11",
                "region_id"=>"11",
                "name"=>"Амударьинский район"
            ],
            [
                "district_id"=>"12",
                "region_id"=>"11",
                "name"=>"Берунийский район"
            ],
            [
                "district_id"=>"13",
                "region_id"=>"11",
                "name"=>"Кандикульский район"
            ],
            [
                "district_id"=>"14",
                "region_id"=>"11",
                "name"=>"Караузякский район"
            ],
            [
                "district_id"=>"15",
                "region_id"=>"11",
                "name"=>"Кегейлийский район"
            ],
            [
                "district_id"=>"16",
                "region_id"=>"11",
                "name"=>"Кунградский район"
            ],
            [
                "district_id"=>"17",
                "region_id"=>"11",
                "name"=>"Муйнакский район"
            ],
            [
                "district_id"=>"18",
                "region_id"=>"11",
                "name"=>"Нукусский район"
            ],
            [
                "district_id"=>"19",
                "region_id"=>"11",
                "name"=>"Тахтакупырский район"
            ],
            [
                "district_id"=>"20",
                "region_id"=>"11",
                "name"=>"Турткульский район"
            ],
            [
                "district_id"=>"21",
                "region_id"=>"11",
                "name"=>"Ходжейлийский район"
            ],
            [
                "district_id"=>"22",
                "region_id"=>"11",
                "name"=>"Чимбайский район"
            ],
            [
                "district_id"=>"23",
                "region_id"=>"11",
                "name"=>"Шуманайский район"
            ],
            [
                "district_id"=>"24",
                "region_id"=>"11",
                "name"=>"Элликкалинский район"
            ],
            [
                "district_id"=>"25",
                "region_id"=>"2",
                "name"=>"город Андижан"
            ],
            [
                "district_id"=>"26",
                "region_id"=>"2",
                "name"=>"город Ханабад"
            ],
            [
                "district_id"=>"27",
                "region_id"=>"2",
                "name"=>"Андижанский район"
            ],
            [
                "district_id"=>"28",
                "region_id"=>"2",
                "name"=>"Балыкчинский район"
            ],
            [
                "district_id"=>"29",
                "region_id"=>"2",
                "name"=>"Булакбашинский район"
            ],
            [
                "district_id"=>"30",
                "region_id"=>"2",
                "name"=>"Бозский район"
            ],
            [
                "district_id"=>"31",
                "region_id"=>"2",
                "name"=>"Джалалкудукский район"
            ],
            [
                "district_id"=>"32",
                "region_id"=>"2",
                "name"=>"Избасканский район"
            ],
            [
                "district_id"=>"33",
                "region_id"=>"2",
                "name"=>"Улугнорский район"
            ],
            [
                "district_id"=>"34",
                "region_id"=>"2",
                "name"=>"Мархаматский район"
            ],
            [
                "district_id"=>"35",
                "region_id"=>"2",
                "name"=>"Пахтаабадский район"
            ],
            [
                "district_id"=>"36",
                "region_id"=>"2",
                "name"=>"Ходжаабадский район"
            ],
            [
                "district_id"=>"37",
                "region_id"=>"2",
                "name"=>"Алтынкульский район"
            ],
            [
                "district_id"=>"38",
                "region_id"=>"2",
                "name"=>"Кургантепинский район"
            ],
            [
                "district_id"=>"39",
                "region_id"=>"2",
                "name"=>"Шахриханский район"
            ],
            [
                "district_id"=>"40",
                "region_id"=>"7",
                "name"=>"город Каган"
            ],
            [
                "district_id"=>"41",
                "region_id"=>"7",
                "name"=>"Бухарский район"
            ],
            [
                "district_id"=>"42",
                "region_id"=>"7",
                "name"=>""
            ],
            [
                "district_id"=>"43",
                "region_id"=>"7",
                "name"=>"Бабкентский район"
            ],
            [
                "district_id"=>"44",
                "region_id"=>"7",
                "name"=>"Джандарский район"
            ],
            [
                "district_id"=>"45",
                "region_id"=>"7",
                "name"=>"Каганский район"
            ],
            [
                "district_id"=>"46",
                "region_id"=>"7",
                "name"=>"Алатский район"
            ],
            [
                "district_id"=>"47",
                "region_id"=>"7",
                "name"=>"Пешкунский район"
            ],
            [
                "district_id"=>"48",
                "region_id"=>"7",
                "name"=>"Ромитанский район"
            ],
            [
                "district_id"=>"49",
                "region_id"=>"7",
                "name"=>"Шафирканский район"
            ],
            [
                "district_id"=>"50",
                "region_id"=>"7",
                "name"=>"Каракульский район"
            ],
            [
                "district_id"=>"51",
                "region_id"=>"7",
                "name"=>"Караулбазарский район"
            ],
            [
                "district_id"=>"52",
                "region_id"=>"7",
                "name"=>"Гиждуванский район"
            ],
            [
                "district_id"=>"53",
                "region_id"=>"9",
                "name"=>"Арнасайский район"
            ],
            [
                "district_id"=>"54",
                "region_id"=>"9",
                "name"=>"Бахмальский район"
            ],
            [
                "district_id"=>"55",
                "region_id"=>"9",
                "name"=>"Галляаральский район"
            ],
            [
                "district_id"=>"56",
                "region_id"=>"9",
                "name"=>"Дустликский район"
            ],
            [
                "district_id"=>"57",
                "region_id"=>"9",
                "name"=>"город Джизак"
            ],
            [
                "district_id"=>"58",
                "region_id"=>"9",
                "name"=>"Жиззах"
            ],
            [
                "district_id"=>"59",
                "region_id"=>"9",
                "name"=>"Зарбдорский район"
            ],
            [
                "district_id"=>"60",
                "region_id"=>"9",
                "name"=>"Зафарабадский район"
            ],
            [
                "district_id"=>"61",
                "region_id"=>"9",
                "name"=>"Заминский район"
            ],
            [
                "district_id"=>"62",
                "region_id"=>"9",
                "name"=>"Мирзачульский район"
            ],
            [
                "district_id"=>"63",
                "region_id"=>"9",
                "name"=>"Пахтакорский район"
            ],
            [
                "district_id"=>"64",
                "region_id"=>"9",
                "name"=>"Фаришский район"
            ],
            [
                "district_id"=>"65",
                "region_id"=>"9",
                "name"=>"Янгиабадский район"
            ],
            [
                "district_id"=>"66",
                "region_id"=>"14",
                "name"=>"город Карши"
            ],
            [
                "district_id"=>"67",
                "region_id"=>"14",
                "name"=>"Каршинский район"
            ],
            [
                "district_id"=>"68",
                "region_id"=>"14",
                "name"=>"Мубарекский район"
            ],
            [
                "district_id"=>"69",
                "region_id"=>"14",
                "name"=>"Гузарский район"
            ],
            [
                "district_id"=>"70",
                "region_id"=>"14",
                "name"=>"Камашинский район"
            ],
            [
                "district_id"=>"71",
                "region_id"=>"14",
                "name"=>"Чиракчинский район"
            ],
            [
                "district_id"=>"72",
                "region_id"=>"14",
                "name"=>"Шахризабский район"
            ],
            [
                "district_id"=>"73",
                "region_id"=>"14",
                "name"=>"Касбинский район"
            ],
            [
                "district_id"=>"74",
                "region_id"=>"14",
                "name"=>"Касанский район"
            ],
            [
                "district_id"=>"75",
                "region_id"=>"14",
                "name"=>"Китабский район"
            ],
            [
                "district_id"=>"76",
                "region_id"=>"14",
                "name"=>"Нишанский район"
            ],
            [
                "district_id"=>"77",
                "region_id"=>"14",
                "name"=>"Миришкорский район"
            ],
            [
                "district_id"=>"78",
                "region_id"=>"14",
                "name"=>"Дехканабадский район"
            ],
            [
                "district_id"=>"79",
                "region_id"=>"14",
                "name"=>"Яккабагский район"
            ],
            [
                "district_id"=>"80",
                "region_id"=>"10",
                "name"=>"город Навойи"
            ],
            [
                "district_id"=>"81",
                "region_id"=>"10",
                "name"=>"город Заравшан"
            ],
            [
                "district_id"=>"82",
                "region_id"=>"10",
                "name"=>"Карманинский район"
            ],
            [
                "district_id"=>"83",
                "region_id"=>"10",
                "name"=>"Тамдынский район"
            ],
            [
                "district_id"=>"84",
                "region_id"=>"10",
                "name"=>"Навбахорский район"
            ],
            [
                "district_id"=>"85",
                "region_id"=>"10",
                "name"=>"Нуратинский район"
            ],
            [
                "district_id"=>"86",
                "region_id"=>"10",
                "name"=>"Хатырчинский район"
            ],
            [
                "district_id"=>"87",
                "region_id"=>"10",
                "name"=>"Кызылтепинский район"
            ],
            [
                "district_id"=>"88",
                "region_id"=>"10",
                "name"=>"Кенимехский район"
            ],
            [
                "district_id"=>"89",
                "region_id"=>"10",
                "name"=>"город Учкудук"
            ],
            [
                "district_id"=>"90",
                "region_id"=>"3",
                "name"=>"город Наманган"
            ],
            [
                "district_id"=>"91",
                "region_id"=>"3",
                "name"=>"Мингбулакский район"
            ],
            [
                "district_id"=>"92",
                "region_id"=>"3",
                "name"=>"Касансайский район"
            ],
            [
                "district_id"=>"93",
                "region_id"=>"3",
                "name"=>"Наманганский район"
            ],
            [
                "district_id"=>"94",
                "region_id"=>"3",
                "name"=>"Нарынский район"
            ],
            [
                "district_id"=>"95",
                "region_id"=>"3",
                "name"=>"Туракурганский район"
            ],
            [
                "district_id"=>"96",
                "region_id"=>"3",
                "name"=>"Уйчинский район"
            ],
            [
                "district_id"=>"97",
                "region_id"=>"3",
                "name"=>"Учкурганский район"
            ],
            [
                "district_id"=>"98",
                "region_id"=>"3",
                "name"=>"Чартакский район"
            ],
            [
                "district_id"=>"99",
                "region_id"=>"3",
                "name"=>"Чустский район"
            ],
            [
                "district_id"=>"100",
                "region_id"=>"3",
                "name"=>"Янгикурганский район"
            ],
            [
                "district_id"=>"101",
                "region_id"=>"8",
                "name"=>"город Самарканд"
            ],
            [
                "district_id"=>"102",
                "region_id"=>"8",
                "name"=>"Ургутсский район"
            ],
            [
                "district_id"=>"103",
                "region_id"=>"8",
                "name"=>"Пахтаачинский район"
            ],
            [
                "district_id"=>"104",
                "region_id"=>"8",
                "name"=>"Каттакурганский район"
            ],
            [
                "district_id"=>"105",
                "region_id"=>"8",
                "name"=>"Самаркандский район"
            ],
            [
                "district_id"=>"106",
                "region_id"=>"8",
                "name"=>"Булунгурский район"
            ],
            [
                "district_id"=>"107",
                "region_id"=>"8",
                "name"=>"Джамбайский район"
            ],
            [
                "district_id"=>"108",
                "region_id"=>"8",
                "name"=>"Кошрабадский район"
            ],
            [
                "district_id"=>"109",
                "region_id"=>"8",
                "name"=>"Нарпайский район"
            ],
            [
                "district_id"=>"110",
                "region_id"=>"8",
                "name"=>"Тайлякский район"
            ],
            [
                "district_id"=>"111",
                "region_id"=>"8",
                "name"=>"Пастдаргомский район"
            ],
            [
                "district_id"=>"112",
                "region_id"=>"8",
                "name"=>"Нуробадский район"
            ],
            [
                "district_id"=>"113",
                "region_id"=>"8",
                "name"=>"город Каттакурган"
            ],
            [
                "district_id"=>"114",
                "region_id"=>"8",
                "name"=>"Пайарыкский район"
            ],
            [
                "district_id"=>"115",
                "region_id"=>"8",
                "name"=>"Акдарьинский район"
            ],
            [
                "district_id"=>"116",
                "region_id"=>"8",
                "name"=>"Иштыханский район"
            ],
            [
                "district_id"=>"117",
                "region_id"=>"13",
                "name"=>"город Термез"
            ],
            [
                "district_id"=>"118",
                "region_id"=>"13",
                "name"=>"Термезский район"
            ],
            [
                "district_id"=>"119",
                "region_id"=>"13",
                "name"=>"Музрабадский район"
            ],
            [
                "district_id"=>"120",
                "region_id"=>"13",
                "name"=>"Алтынсайский район"
            ],
            [
                "district_id"=>"121",
                "region_id"=>"13",
                "name"=>"Денаусский район"
            ],
            [
                "district_id"=>"122",
                "region_id"=>"13",
                "name"=>"Сарыассийский район"
            ],
            [
                "district_id"=>"123",
                "region_id"=>"13",
                "name"=>"Кизирикский район"
            ],
            [
                "district_id"=>"124",
                "region_id"=>"13",
                "name"=>"Джаркурганский район"
            ],
            [
                "district_id"=>"125",
                "region_id"=>"13",
                "name"=>"Ангорский район"
            ],
            [
                "district_id"=>"126",
                "region_id"=>"13",
                "name"=>"Кумкурганский район"
            ],
            [
                "district_id"=>"127",
                "region_id"=>"13",
                "name"=>"Байсунский район"
            ],
            [
                "district_id"=>"128",
                "region_id"=>"13",
                "name"=>"Шурчинский район"
            ],
            [
                "district_id"=>"129",
                "region_id"=>"13",
                "name"=>"Шерабадский район"
            ],
            [
                "district_id"=>"130",
                "region_id"=>"13",
                "name"=>"Узунский район"
            ],
            [
                "district_id"=>"131",
                "region_id"=>"12",
                "name"=>"город Гулистан"
            ],
            [
                "district_id"=>"132",
                "region_id"=>"12",
                "name"=>"город Янгиер"
            ],
            [
                "district_id"=>"133",
                "region_id"=>"12",
                "name"=>"город Ширин"
            ],
            [
                "district_id"=>"134",
                "region_id"=>"12",
                "name"=>"Окалтынский район"
            ],
            [
                "district_id"=>"135",
                "region_id"=>"12",
                "name"=>"Баяутский район"
            ],
            [
                "district_id"=>"136",
                "region_id"=>"12",
                "name"=>"Гулистанский район"
            ],
            [
                "district_id"=>"137",
                "region_id"=>"12",
                "name"=>"Мирзаабадский район"
            ],
            [
                "district_id"=>"138",
                "region_id"=>"12",
                "name"=>"Сайхунабадский район"
            ],
            [
                "district_id"=>"139",
                "region_id"=>"12",
                "name"=>"Сардобинский район"
            ],
            [
                "district_id"=>"140",
                "region_id"=>"12",
                "name"=>"Сырдарьинский район"
            ],
            [
                "district_id"=>"141",
                "region_id"=>"12",
                "name"=>"Хавастский район"
            ],
            [
                "district_id"=>"142",
                "region_id"=>"5",
                "name"=>"город Ангрен"
            ],
            [
                "district_id"=>"143",
                "region_id"=>"5",
                "name"=>"город Бекабад"
            ],
            [
                "district_id"=>"144",
                "region_id"=>"5",
                "name"=>"город Алмалык"
            ],
            [
                "district_id"=>"145",
                "region_id"=>"5",
                "name"=>"город Чирчик"
            ],
            [
                "district_id"=>"146",
                "region_id"=>"5",
                "name"=>"Бекабадский район"
            ],
            [
                "district_id"=>"147",
                "region_id"=>"5",
                "name"=>"Бостанлыкский район"
            ],
            [
                "district_id"=>"148",
                "region_id"=>"5",
                "name"=>"Кибрайский район"
            ],
            [
                "district_id"=>"149",
                "region_id"=>"5",
                "name"=>"Зангиатинский район"
            ],
            [
                "district_id"=>"150",
                "region_id"=>"5",
                "name"=>"Куйи-чирчикский район"
            ],
            [
                "district_id"=>"151",
                "region_id"=>"5",
                "name"=>"Аккурганский район"
            ],
            [
                "district_id"=>"152",
                "region_id"=>"5",
                "name"=>"Паркентский район"
            ],
            [
                "district_id"=>"154",
                "region_id"=>"5",
                "name"=>"Урта-чирчикский район"
            ],
            [
                "district_id"=>"155",
                "region_id"=>"5",
                "name"=>"Юкори чирчикский район"
            ],
            [
                "district_id"=>"156",
                "region_id"=>"5",
                "name"=>"Юкори чирчикский район"
            ],
            [
                "district_id"=>"157",
                "region_id"=>"5",
                "name"=>"Букинский район"
            ],
            [
                "district_id"=>"158",
                "region_id"=>"5",
                "name"=>"Янгийульский район"
            ],
            [
                "district_id"=>"159",
                "region_id"=>"5",
                "name"=>"Ахангаранский район"
            ],
            [
                "district_id"=>"160",
                "region_id"=>"15",
                "name"=>"город Фергана"
            ],
            [
                "district_id"=>"161",
                "region_id"=>"15",
                "name"=>"город Маргилан"
            ],
            [
                "district_id"=>"162",
                "region_id"=>"15",
                "name"=>"город Кувасай"
            ],
            [
                "district_id"=>"163",
                "region_id"=>"15",
                "name"=>"город Коканд"
            ],
            [
                "district_id"=>"164",
                "region_id"=>"15",
                "name"=>"Багдадский район"
            ],
            [
                "district_id"=>"165",
                "region_id"=>"15",
                "name"=>"Бувайдинский район"
            ],
            [
                "district_id"=>"166",
                "region_id"=>"15",
                "name"=>"Дангаринский район"
            ],
            [
                "district_id"=>"167",
                "region_id"=>"15",
                "name"=>"Язьяванский район"
            ],
            [
                "district_id"=>"168",
                "region_id"=>"15",
                "name"=>"Алтыарыкский район"
            ],
            [
                "district_id"=>"169",
                "region_id"=>"15",
                "name"=>"Бешарыкский район"
            ],
            [
                "district_id"=>"170",
                "region_id"=>"15",
                "name"=>"Куштепинский район"
            ],
            [
                "district_id"=>"171",
                "region_id"=>"15",
                "name"=>"Риштанский район"
            ],
            [
                "district_id"=>"172",
                "region_id"=>"15",
                "name"=>"Сохский район"
            ],
            [
                "district_id"=>"173",
                "region_id"=>"15",
                "name"=>"Ташлакский район"
            ],
            [
                "district_id"=>"174",
                "region_id"=>"15",
                "name"=>"Учкуприкский район"
            ],
            [
                "district_id"=>"175",
                "region_id"=>"15",
                "name"=>"Ферганский район"
            ],
            [
                "district_id"=>"176",
                "region_id"=>"15",
                "name"=>"Узбекистанский район"
            ],
            [
                "district_id"=>"177",
                "region_id"=>"15",
                "name"=>"Кувинский район"
            ],
            [
                "district_id"=>"178",
                "region_id"=>"15",
                "name"=>"Фуркатский район"
            ],
            [
                "district_id"=>"179",
                "region_id"=>"4",
                "name"=>"город Ургенч"
            ],
            [
                "district_id"=>"180",
                "region_id"=>"4",
                "name"=>"Багатский район"
            ],
            [
                "district_id"=>"181",
                "region_id"=>"4",
                "name"=>"Ургенчский район"
            ],
            [
                "district_id"=>"182",
                "region_id"=>"4",
                "name"=>"Кушкупырский район"
            ],
            [
                "district_id"=>"183",
                "region_id"=>"4",
                "name"=>"Ханкинский район"
            ],
            [
                "district_id"=>"184",
                "region_id"=>"4",
                "name"=>"Янгиарыкский район"
            ],
            [
                "district_id"=>"185",
                "region_id"=>"4",
                "name"=>"Хивинский район"
            ],
            [
                "district_id"=>"186",
                "region_id"=>"4",
                "name"=>"Янгибазарский район"
            ],
            [
                "district_id"=>"187",
                "region_id"=>"4",
                "name"=>"Хазараспский район"
            ],
            [
                "district_id"=>"188",
                "region_id"=>"4",
                "name"=>"Шаватский район"
            ],
            [
                "district_id"=>"189",
                "region_id"=>"4",
                "name"=>"Гурленский район"
            ],
            [
                "district_id"=>"190",
                "region_id"=>"6",
                "name"=>"Бектемирский район"
            ],
            [
                "district_id"=>"191",
                "region_id"=>"6",
                "name"=>"Мирабадский район"
            ],
            [
                "district_id"=>"192",
                "region_id"=>"6",
                "name"=>"М.Улугбекский район"
            ],
            [
                "district_id"=>"193",
                "region_id"=>"6",
                "name"=>"Сергелийсикй район"
            ],
            [
                "district_id"=>"194",
                "region_id"=>"6",
                "name"=>"Алмазарский район"
            ],
            [
                "district_id"=>"195",
                "region_id"=>"6",
                "name"=>"Учтепинский район"
            ],
            [
                "district_id"=>"196",
                "region_id"=>"6",
                "name"=>"Яшнабадский район"
            ],
            [
                "district_id"=>"197",
                "region_id"=>"6",
                "name"=>"Чилонзарский район"
            ],
            [
                "district_id"=>"198",
                "region_id"=>"6",
                "name"=>"Шайхантахурский район"
            ],
            [
                "district_id"=>"199",
                "region_id"=>"6",
                "name"=>"Юнусабадский район"
            ],
            [
                "district_id"=>"200",
                "region_id"=>"6",
                "name"=>"Яккасарайский район"
            ],
            [
                "district_id"=>"201",
                "region_id"=>"5",
                "name"=>"Пскентский район"
            ]
        ];

        foreach ($districts as $item) {
            District::query()->firstOrCreate(['district_id' => $item['district_id']], [
                'district_id' => $item['district_id'],
                'region_id' => $item['region_id'],
                'name' => $item['name']
            ]);
        }
    }
}
